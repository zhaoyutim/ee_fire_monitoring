...
class PatchExpanding(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        x = depth_to_space(x, 2, data_format='NHWC')
        x = tf.reshape(x, (-1, L*2*2, C//2))

        return x

class PatchRestoration(tf.keras.layers.Layer):
    def __init__(self, input_resolution, dim, patch_size, prefix=''):
        super(PatchRestoration, self).__init__()
    ...
    def call(self, x):
        ...
        # rearange depth to number of patches
        x = depth_to_space(x, self.patch_size, data_format='NHWC')
...
class BasicLayer(tf.keras.layers.Layer):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        # build blocks
        self.blocks = tf.keras.Sequential([SwinTransformerBlock(dim=dim * 2 if downsample is not None else dim, input_resolution=(input_resolution[0]//2, input_resolution[1]//2) if downsample is not None else input_resolution,
...
class DecoderLayer(tf.keras.layers.Layer):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path_prob=0., norm_layer=LayerNormalization, upsample=None, use_checkpoint=False, prefix=''):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.depth = depth
...
class SwinTransformerModel(tf.keras.Model):
    def __init__(self, model_name='swin_tiny_patch4_window7_224', include_top=False,
        ...
        self.decoder_num_heads = num_heads[::-1]
        ...
        # build layers
        self.basic_layers = [BasicLayer(dim=int(embed_dim * 2 ** (i_layer-1)) if i_layer > 0 else embed_dim,
        ...
        self.decoder_layers = [DecoderLayer(dim=int(embed_dim * 2 ** i_layer),
    ...
    def forward_features(self, x):
        ...
        x = self.pos_drop(x)
        x = self.basic_layers[0](x)
        skip_x1 = x
        x = self.basic_layers[1](x)
        skip_x2 = x
        x = self.basic_layers[2](x)
        skip_x3 = x
